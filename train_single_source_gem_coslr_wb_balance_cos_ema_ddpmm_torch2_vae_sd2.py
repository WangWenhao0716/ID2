...
from dg.trainers_cos_ema_ddpmm_vae_sd2 import Trainer
...
#from isc.io import read_descriptors, read_descriptors_single
#import h5py
...
def main_worker(world_size, args):
    ...
    '''
    training_names, training_features = read_descriptors_single('../Benchmark/feature/sd2/training_vae.hdf5')
    #read_descriptors(['../Benchmark/feature/sd2/training_%d_vae_fix.hdf5'%i for i in range(210)])
    training_features = torch.Tensor(training_features).cuda(device)
    training_names = np.array(training_names)
    '''
    #hh = h5py.File('../Benchmark/feature/sd2/training_vae.hdf5', "r")
    ...
    #[]['../Benchmark/feature/sd2/training_%d_vae_fix.hdf5'%i for i in range(42)]
    ...
    #training_features = torch.load('training_features.pt')#.cuda(device)
    #training_features = torch.load('training_features_sdxl.pt')
    #training_features = torch.load('training_features_sdxl_norm.pt')
    #training_features = torch.load('training_features_sd3.pt')
