...
class Trainer(object):
    ...
    def train(self, epoch, data_loader, optimizer, ema, train_iters=200, print_freq=1, rank=0):
        ...
        for i in range(train_iters):
            ...
            inputs, targets = self._parse_data(source_inputs, self.gpu)
            s_features, s_cls_out= self.model(inputs, targets)
            
            '''
            camids, targets = self._parse_data(source_inputs, self.gpu)
            training_features_select = training_features[camids].cuda()
            s_features, s_cls_out= self.model(training_features_select, targets)
            '''

            # backward main #
            #loss_ce, loss_tr, prec1 = self._forward(s_features, s_cls_out, targets)
            loss_ce, prec = self._forward(s_features, s_cls_out, targets)
    ...
    def _parse_data(self, inputs, gpu):
        imgs, img_names, pids, camid = inputs
        #img_names = [i.split('/')[-1].split('.')[0] for i in img_names]
